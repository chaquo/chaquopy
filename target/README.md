...
give it. "bX" and "rcX" suffixes should be included in the version, not the build
number. Once a version has been published on Maven Central, it cannot be changed, so any
fixes must be released under a different build number.

If building Python 3.12 or older, consider updating the cpython-android-source-deps
library versions in python/build.sh.

Make sure the build machine has `pythonX.Y` on the PATH, where `X.Y` is the
major.minor version you want to build. The micro version doesn't matter.
