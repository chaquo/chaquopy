    ...
    def __str__(self):
        ...
        if self.req:
            ...
            if self.link:
                ...
                s += ' from %s' % redact_password_from_url(self.link.url_without_fragment)
        elif self.link:
            s = redact_password_from_url(self.link.url_without_fragment)
        else:
        ...
        if hasattr(self.link, "chaquopy_candidates"):
            wheel_versions = [str(ver) for ver in
                              sorted(set(candidate.version
                                         for candidate in self.link.chaquopy_candidates.iter_all()
