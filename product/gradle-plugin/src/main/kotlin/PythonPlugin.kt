...
class PythonPlugin @Inject constructor(
    ...
    fun findPlugin(group: String, name: String): PluginInfo {
        // Search all the places the plugin may be declared.
        val buildscripts = sequence {
            ...
            // Projects generated by Flutter declare all of their plugins in the
            // top-level `plugins` block of settings.gradle, not within pluginManagement
            // (https://docs.flutter.dev/release/breaking-changes/flutter-gradle-plugin-apply)
            // Unfortunately we need to use an internal API to access that. See:
            // * https://github.com/chaquo/chaquopy/issues/1289
            // * https://github.com/gradle/gradle/issues/17295
            yield((project.gradle as GradleInternal).settings.buildscript)
        }

        for (buildscript in buildscripts) {
            val classpath = buildscript.configurations.getByName("classpath")
            for (art in classpath.resolvedConfiguration.resolvedArtifacts) {
                val dep = art.moduleVersion.id
                if (dep.group == group  &&  dep.name == name) {
                    return PluginInfo(buildscript, Version.parse(dep.version))
                }
            }
        }
